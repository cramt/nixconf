...
- **Packages**: `packages/` — custom packages (`agent-browser`, `agentsview`, `cockatrice`, `colibri`, `declaradroid`, `rhystic-tracker`, `saturn-windows-image`, `steamlink`, `t3code`). `packages/rhystic-tracker/python/` is a small local python package set (UnityPy + its codec deps, none in nixpkgs) wired onto that app's PATH; each member is a flake output so `nix-update` can bump it.
