# NixOS general bundle — locale, stylix, nix helpers, udev
{ inputs, ... }: {
  flake.nixosModules."bundles.general" = { config, lib, pkgs, ... }:
  let
  ...
  in {
    options.myNixOS.bundles.general = {
      enable = lib.mkEnableOption "myNixOS.bundles.general";
