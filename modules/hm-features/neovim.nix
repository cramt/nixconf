          ...
          vim = {
            luaConfigRC.no_fill_chars = ''
              ...
              if not vim.g.neovide then
                local groups = { "Normal", "NormalFloat", "NormalNC", "SignColumn", "LineNr", "CursorLineNr", "EndOfBuffer", "FloatBorder", "WinSeparator", }
            ...
            '';
            globals = { neovide_opacity = 0.8; guifont = "Iosevka Nerd Font"; };
            viAlias = true; vimAlias = true; lineNumberMode = "number";
            clipboard = { enable = true; registers = "unnamedplus"; };
            debugMode = { enable = false; level = 16; logFile = "/tmp/nvim.log"; };
            spellcheck = { enable = true; };
            keymaps = [
              { key = "<leader>w"; mode = "n"; action = "<Cmd>w<CR>"; desc = "Save"; }
              { key = "<leader>q"; mode = "n"; action = "<Cmd>confirm q<CR>"; desc = "Quit"; }
              { key = "<leader>Q"; mode = "n"; action = "<Cmd>confirm qall<CR>"; desc = "Quit all"; }
              { key = "<leader>c"; mode = "n"; lua = true; desc = "Close buffer";
                action = ''
                  function()
                    require("bufdelete").bufdelete(vim.api.nvim_get_current_buf(), false)
                  end
                ''; }
              { key = "<leader>C"; mode = "n"; lua = true; desc = "Force close buffer";
                action = ''
                  function()
                    require("bufdelete").bufdelete(vim.api.nvim_get_current_buf(), true)
                  end
                ''; }
              { key = "L"; mode = "n"; action = "<Cmd>bnext<CR>"; desc = "Next Buffer"; }
              { key = "H"; mode = "n"; action = "<Cmd>bprev<CR>"; desc = "Prev Buffer"; }
              { key = "<C-Left>"; mode = "n"; action = ":vertical resize -5<CR>"; desc = "Resize Left"; }
              { key = "<C-Right>"; mode = "n"; action = ":vertical resize +5<CR>"; desc = "Resize Right"; }
              { key = "<leader>e"; mode = "n"; lua = true; desc = "Open exlorer";
                action = ''
                  function()
                ...
                ''; }
              { key = "<Tab>"; mode = "v"; action = ">gv"; desc = "Indent"; }
            ...
            filetree.neo-tree = {
              ...
              setupOpts = {
                ...
                filesystem = { follow_current_file = { enabled = true; leave_dirs_open = true; }; hijack_netrw_behavior = "open_current"; filtered_items = { visible = false; hide_dotfiles = false; hide_gitignored = true; hide_by_pattern = [".git" ".jj"]; }; };
              };
            };
            ...
            ui = { borders.enable = true; noice.enable = true; colorizer.enable = true; modes-nvim.enable = false; illuminate.enable = true; };
            assistant.copilot = { enable = false; cmp.enable = false; };
