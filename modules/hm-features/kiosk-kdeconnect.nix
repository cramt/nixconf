{ ... }: {
  hmModules.features.kiosk-kdeconnect = { config, lib, pkgs, ... }: let
  ...
  in {
    options.myHomeManager.kiosk-kdeconnect = {
      enable = lib.mkEnableOption "myHomeManager.kiosk-kdeconnect";
      commands = lib.mkOption {
        type = lib.types.attrsOf (lib.types.submodule {
          options = {
            name = lib.mkOption { type = lib.types.str; description = "Display name for the command"; };
            command = lib.mkOption { type = lib.types.str; description = "Shell command to execute"; };
          };
        });
        default = {};
    ...
    config = lib.mkIf config.myHomeManager.kiosk-kdeconnect.enable {
