{ ... }: {
  hmModules.features.mpvpaper = { config, lib, pkgs, ... }: let
  ...
  in {
    options.myHomeManager.mpvpaper = {
      enable = lib.mkEnableOption "myHomeManager.mpvpaper";
      backgroundVideo = lib.mkOption {
        type = lib.types.path;
      };
    };
    config = lib.mkIf config.myHomeManager.mpvpaper.enable {
      systemd.user.services =
        lib.mapAttrs' (name: value: {
          name = "mpvpaper_${builtins.hashString "md5" name}";
          value = {
            Unit = { Description = "Start animated background on boot"; After = ["graphical.target"]; };
