...
**Balance:** The bar is triviality, not repetition. A one-off still earns a lever when the lever is what makes the work checkable. Per the [Laziness Protocol](../laziness-protocol/SKILL.md), build the smallest script that does or proves the job, never a framework.

Distinct from encoding a recurring instruction as a durable guardrail (a lint, a metadata flag, a runtime check). This is throughput and reviewability on the work in front of you. The same move applies to verification: script the check against the real artifact rather than re-running it by hand.
