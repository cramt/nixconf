...
Each unit's check must be real: verify against the artifact itself, not a proxy or "it compiles". The **build-the-lever** skill is what makes the per-unit check cheap enough to actually run every time.
