# Titan VM orchestration
{ inputs, ... }: {
  flake.nixosModules."services.titan-vm" = { config, lib, pkgs, ... }:
  let
  ...
  in {
    options.myNixOS.services.titan-vm = {
      enable = lib.mkEnableOption "myNixOS.services.titan-vm";
      dataDir = lib.mkOption { type = lib.types.str; default = "/var/lib/titan-vm"; description = "Directory to store VM disk image and state"; };
      memory = lib.mkOption { type = lib.types.str; default = "2G"; description = "Amount of RAM for the VM"; };
      cores = lib.mkOption { type = lib.types.int; default = 2; description = "Number of CPU cores for the VM"; };
      secretFiles = lib.mkOption {
        type = lib.types.attrsOf lib.types.str; default = {};
        description = "Attrset of filename -> host path for secrets shared into the VM.";
      };
    };
    config = lib.mkIf cfg.enable {
      ...
      systemd.services.titan-vm = {
        ...
        environment = {
          QEMU_OPTS = lib.concatStringsSep " " [
            "-m ${cfg.memory}" "-smp ${toString cfg.cores}" "-nographic"
      ...
      myNixOS.services.caddy.serviceMap.openclaw = { inherit port; };
