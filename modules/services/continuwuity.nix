{ ... }: {
  flake.nixosModules."services.continuwuity" = {
    config,
    lib,
    ...
  }: let
    cfg = config.myNixOS.services.continuwuity;
    site = import ../../myLib/site.nix;
    turnDomain = "turn.${site.domain}";
    turnMin = 49000;
    turnMax = 50000;
  in {
    options.myNixOS.services.continuwuity = {
      enable = lib.mkEnableOption "myNixOS.services.continuwuity";
    };
