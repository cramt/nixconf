# Valheim game server
{ ... }: {
  flake.nixosModules."services.valheim" = { config, lib, pkgs, ... }:
  let
    cfg = config.myNixOS.services.valheim;
    docker_source = pkgs.npinsSources."mbround18/valheim";
  in {
    options.myNixOS.services.valheim = {
      enable = lib.mkEnableOption "myNixOS.services.valheim";
      worldVolume = lib.mkOption { type = lib.types.str; description = "world volume mount"; };
      binaryVolume = lib.mkOption { type = lib.types.str; description = "binary volume mount"; };
      serverName = lib.mkOption { type = lib.types.str; description = "server name"; };
      worldName = lib.mkOption { type = lib.types.str; description = "world name"; };
    };
    config = lib.mkIf cfg.enable {
      ...
      virtualisation.oci-containers.containers.valheim = {
        ...
        environment = {
          PUID = "111"; PGID = "1000"; TZ = "Europe/Copenhagen";
          PORT = "2456"; NAME = cfg.serverName; WORLD = cfg.worldName;
          PUBLIC = "0"; AUTO_UPDATE = "0"; TYPE = "BepInEx";
          MODS = ''
        ...
        environmentFiles = [ config.services.onepassword-secrets.secretPaths.valheimEnv ];
        extraOptions = [ "--shm-size=2gb" "--cpu-shares=10" ];
